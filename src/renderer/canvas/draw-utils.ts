...
export function drawShadowNode(ctx: CanvasRenderingContext2D, agentX: number, agentY: number, insight: { kind: string; summary?: string; confidence?: number; details?: string }, time: number) {
  const x = agentX + 88;
  const y = agentY - 62;
