/**
 * Exhibit vocabulary — the typed artifact contracts the curator fills.
 *
 * v1 types from `docs/plans/req-v1-curator.md` §6 (recovered from the
 * exhibit-floor pin / `feat/exhibit-components`): seven authored exhibits plus
 * `live_graph`, which wraps agent-flow's living canvas so it can ride in the
 * same rotation. Every artifact is an {@link ExhibitArtifact} envelope
 * (id, narrative, relevance, decay, status) with a payload discriminated by
 * `exhibitType`.
 */
